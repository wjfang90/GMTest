...
namespace KYSharp.SM.Core.Test.SM.SM3 {
    public class SM3Digest : GeneralDigest {
        ...
        public static string DoHash(string input, Encoding encoding) {
            byte[] md = new byte[32];
            byte[] msg = encoding.GetBytes(input);
            SM3Digest sm3 = new SM3Digest();
            sm3.BlockUpdate(msg, 0, msg.Length);
            sm3.DoFinal(md, 0);
            string res = new UTF8Encoding().GetString(Hex.Encode(md));
            //var res1 = encoding.GetString(Hex.Encode(md));
